...
class PetastormLoader:
    def __init__(self, video_metadata: VideoMetaInfo, batch_size=1,
                 skip_frames=0, offset=None, limit=None, cur_shard=None, 
                 shard_count=None, num_epochs = 1):
        """
    ...
    def load(self):
        ...
        with make_reader(self.video_metadata.file, cur_shard=self.cur_shard, \
            shard_count=self.shard_count, num_epochs=self.num_epochs) as reader:
            frames = []

            for frame_ind, row in enumerate(reader):

                if self.offset and frame_ind < self.offset:
                    continue
